...
fn count_iterator(map: &HashMap<String, Progress>, value: Progress) -> usize {
    ...
    let filteredmap : Vec<(&String, &Progress)> = map.iter().filter(|elem| elem.1 == &value).collect();
    filteredmap.len()
}
...
fn count_collection_iterator(collection: &[HashMap<String, Progress>], value: Progress) -> usize {
    ...
    collection.iter().map(|hashmap| {
        let filteredmap : Vec<(&String, &Progress)> = hashmap.iter().filter(|elem| elem.1 == &value).collect();
        filteredmap.len()
    }
    ).sum()
}
